  ...
  if (loading) {
    return (
      <div className="p-12 text-center animate-pulse">
        <div className="h-8 bg-slate-200 dark:bg-slate-800 rounded w-3/4 mx-auto mb-6"></div>
        <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded w-full mx-auto mb-3"></div>
        <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded w-5/6 mx-auto mb-3"></div>
        <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded w-4/6 mx-auto"></div>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="p-12 text-center">
        <h2 className="text-2xl font-serif font-bold text-slate-800 dark:text-slate-200 mb-2">Conteúdo não encontrado</h2>
        <p className="text-slate-500 dark:text-slate-400">O módulo ou conferência solicitada ({moduloId}/{conferenciaId}) não existe.</p>
      </div>
    );
  }

  return (
    <div className="p-6 md:p-10 lg:p-14 max-w-5xl mx-auto animate-in fade-in duration-500">
      ...
      {data.objetivos && data.objetivos.length > 0 && (
        <div className="bg-light-surface dark:bg-dark-surface border-4 border-zinc-900 dark:border-zinc-100 shadow-[6px_6px_0px_#ec4899] dark:shadow-[6px_6px_0px_#ec4899] p-6 lg:p-8 mb-12">
          <h3 className="font-serif font-black uppercase text-zinc-900 dark:text-white mb-6 flex items-center gap-3 text-xl tracking-tight">
            <span className="bg-primary text-white px-2 py-1 border-2 border-zinc-900 dark:border-white shadow-[2px_2px_0px_rgba(0,0,0,1)] dark:shadow-[2px_2px_0px_rgba(255,255,255,1)]">OBJ</span> Objetivos de Aprendizagem
          </h3>
          <ul className="space-y-3 text-zinc-800 dark:text-zinc-200 font-medium text-lg">
            {data.objetivos.map((obj, i) => (
              <li key={i} className="flex gap-4 items-start">
                <span className="text-primary font-black text-xl leading-none mt-1">→</span> 
                <span>{obj}</span>
              </li>
            ))}
          </ul>
        ...
        <div className="mt-20 pt-12 border-t-8 border-zinc-900 dark:border-zinc-100">
          ...
          <div className="grid gap-8">
            {data.ejercicios.map((ej, i) => (
              <div key={i} className="bg-light-bg dark:bg-dark-bg p-6 md:p-8 border-4 border-zinc-900 dark:border-zinc-100 shadow-[8px_8px_0px_#06b6d4] dark:shadow-[8px_8px_0px_#06b6d4] relative">
