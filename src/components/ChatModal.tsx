...
export function ChatModal() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: `Olá! Sou seu assistente de IA da disciplina ${COURSE_NAME}. Como posso ajudar você no seu aprendizado hoje?` }
  ]);
    ...
    try {
      ...
      if (apiKey) {
        const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`, {
          ...
          body: JSON.stringify({
            system_instruction: { 
              parts: [{ text: "Você é um tutor de programação prestativo focando no uso do runtime Bun.js. Você está ajudando alunos de 4º ano universitário na disciplina Opcional 2. Mantenha as respostas concisas, técnicas mas pedagogicamente claras." }] 
            },
        ...
        aiResponseText = data?.candidates?.[0]?.content?.parts?.[0]?.text || "Desculpe, não consegui processar a resposta.";
      } else {
         await new Promise(r => setTimeout(r, 800));
         aiResponseText = "Esta é uma resposta de demonstração. Defina a variável VITE_GEMINI_API_KEY no seu ambiente preencher com a chave real da API Gemini.";
      }

      setMessages(prev => [...prev, { role: 'assistant', content: aiResponseText }]);
    } catch {
      setMessages(prev => [...prev, { role: 'assistant', content: 'Ops! Ocorreu um erro ao comunicar com a IA. Tente novamente mais tarde.' }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 w-85 md:w-100 h-137.5 md:h-150 max-h-[85vh] border-4 border-zinc-900 dark:border-zinc-100 shadow-[8px_8px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_rgba(255,255,255,0.2)] bg-light-bg dark:bg-dark-bg flex flex-col z-50 overflow-hidden animate-in slide-in-from-bottom-5">
      <div className="flex items-center justify-between p-4 border-b-4 border-zinc-900 dark:border-zinc-100 bg-secondary dark:bg-secondary text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs opacity-90 font-medium">Assistente do curso</p>
          </div>
        </div>
        <button onClick={() => setIsOpen(false)} className="text-white hover:text-zinc-900 dark:hover:text-black transition-colors p-1.5 border-2 border-transparent hover:border-zinc-900 dark:hover:border-zinc-100 hover:bg-white">
          <X className="w-6 h-6" />
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t-4 border-zinc-900 dark:border-zinc-100 bg-light-bg dark:bg-dark-bg flex gap-3">
        <input
          type="text"
          value={input}
